...
Typical usage
-------------
1) Vectors-only (no dependency on Protein_Folding runtime):
    from qsadpp.reverse_decoder import ReverseDecoder
    dec = ReverseDecoder()
    out = dec.decode_vectors(bitstring, side_chain_hot_vector, fifth_bit)

2) Full decoding with your ProteinFoldingProblem:
    problem = build_problem(sequence)
    from qsadpp.reverse_decoder import ReverseDecoder, safe_get_calpha_coords
    dec = ReverseDecoder(calpha_getter=safe_get_calpha_coords)
    coords, vectors = dec.decode_coords_via_problem(problem, bitstring,
                                                   side_chain_hot_vector, fifth_bit)
"""
...
# -------------------------------
# Safe access to Cα coordinates from your ProteinFoldingResult
# -------------------------------
...
def safe_get_calpha_coords(pf_result) -> np.ndarray:
    ...
    # Try to extract Cα with common keys ("CA", "Calpha", "Cα", "C-A")
        ...
        if isinstance(obj, dict):
            for k in keys:
                if k in obj:
                    ...
                    if arr.ndim == 2 and arr.shape[1] == 3:
                        return arr
        # If it looks like a tuple/list of arrays, pick the first as Cα (best-effort)
...
class ReverseDecoder:
    ...
    # ---- Vectors-only decoding (uses the unmodified ProteinShapeDecoder) ----
    def decode_vectors(
        self,
        bitstring: str,
    ...
    # ---- Full decoding via ProteinFoldingProblem -> ProteinFoldingResult -> Cα ----
    def decode_coords_via_problem(
        ...
        # Build a singleton probability map {bitstring: 1.0} and interpret
...
# -------------------------------
# Convenience: batch decode for a group (vectors-only or full)
# -------------------------------
