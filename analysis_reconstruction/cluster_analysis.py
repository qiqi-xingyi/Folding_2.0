...
def parse_prefilter(df: pd.DataFrame, rules: Dict[str, Any]) -> pd.Series:
    """Apply simple prefilter rules. Examples: {"E_steric<=0": true, "E_geom<=7.5": true}"""
    if not rules:
...
def pick_best_k_by_silhouette(dm: np.ndarray, rng: np.random.RandomState, k_candidates: Sequence[int]) -> Tuple[int, Dict[int, float]]:
    scores: Dict[int, float] = {}
    best_k = None
    best_score = -1.0
    for k in k_candidates:
        labels, _ = pam_kmedoids(dm, k, rng)
        score = silhouette_score_from_distance(dm, labels)
        scores[k] = score
        if score > best_score:
            best_score = score
            best_k = k
...
class ClusterAnalyzer:
    """Cluster on main_vectors with Hamming distance and pick the best-energy cluster."""
    ...
    def fit(self):
        ...
        if n < 2:
            ...
            if self.cfg.method == "hdbscan":
                if not _HDBSCAN_AVAILABLE:
                    logging.warning("hdbscan not available, falling back to kmedoids.")
                    self.cfg.method = "kmedoids"
                else:
                    min_cs = self.cfg.min_cluster_size or max(5, int(math.sqrt(n)))
                    self.labels_ = cluster_with_hdbscan(dm, min_cs)
            if self.cfg.method == "kmedoids":
                best_k, _ = pick_best_k_by_silhouette(dm, self.rng, self.cfg.k_candidates)
                labels, _ = pam_kmedoids(dm, best_k, self.rng)
                self.labels_ = labels
