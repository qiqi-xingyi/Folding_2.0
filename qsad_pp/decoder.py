...
class ProteinShapeDecoder:
    ...
    def __init__(
        self, vector_sequence: str, side_chain_hot_vector: List[bool], fifth_bit: bool
    ) -> None:
        """
        Args:
            vector_sequence: Sequence to be decoded.
        ...
        self._main_vectors = self._get_main_vectors()
        self._side_vectors = self._get_side_vectors()

    @property
    def main_vectors(self):
        """Returns the turns of the main chain."""
        return self._main_vectors

    @property
    def side_vectors(self):
        """Returns the turns of the side chain."""
        return self._side_vectors

    def _bitstring_to_vectors(self, bitstring: str) -> List[int]:
        """
        Takes a bitstring encoding the vectors of a chain and returns the vectors as a list of integers.
        ...
        length_vectors = len(bitstring) // 2
        return [encoding[bitstring[2 * i : 2 * (i + 1)]] for i in range(length_vectors)]
    ...
    def _get_main_vectors(self) -> List[int]:
        """
        ...
        main_turns_bitstring = (
                self._vector_sequence[-self._split_bitstring()[0]:] + "0010"
        )
        ...
        return self._bitstring_to_vectors(main_turns_bitstring)

    def _get_side_vectors(self) -> List[Optional[int]]:
        """
        ...
        side_turns_bitstring = self._vector_sequence[-n - m: -n]

        side_turns = self._bitstring_to_vectors(side_turns_bitstring)
